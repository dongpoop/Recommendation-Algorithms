...
def test_accuracy(number, train_ratio_of_general_table, sim_threshold, general_table, top_n):
    ...
    for no in range(number):
        sql_create_sample_table = f'create table test_svd{no + 1}(' \
        ...
        table_list.append(f'test_svd{no + 1}')
    print('sample successfully :')
    print(table_list)
    # test accuracy of every sample
    for no in range(number):
        print(f'test_svd sample {no + 1}...')
        # get every user id of every table
        sql_select_user_id_of_one_table = f'select distinct (userId + 0) from test_svd{no + 1};'
        ...
        # compute similarity
        sim_path = cf.compute_similarity_using_for_user_SVD('root', 'root', 3306, 'movies', f'test_svd{no + 1}',
                                                        'localhost', 'utf8', 1.0, 'adjusted_cos')
        # find all movie ids of table
        sql_select_movie_id_of_one_table = f'select distinct (movieId + 0) from test_svd{no + 1};'
        ...
        for user_id in user_id_list:
            ...
            recommendation_list = cf.recommend(user_id, '1995-01-09 00:00:00', '2019-11-21 00:00:00',
                                               sim_threshold,
                                               sim_path, top_n, f'test_svd{no + 1}')
        ...
        print(f'test_svd score {no}: {score_list[-1]}')
        print(f'test_svd quality {no}: {quality_list[-1]}')
